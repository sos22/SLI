...
rm enforce_crash_*
for crash_summary in crash_summaries/*
do
    ...
    echo -n "Start testing patch at "
    date +%s

    export SOS22_ENFORCER_MAX_STALLS=10000
    cd /local/scratch/sos22/mysql-5.5.6-rc/mysql-test
    rm -rf var
    script -f -c "./mtr --testcase-timeout=99999 --max-test-fail=1 rpl_change_master" ${topdir}/logs/mtr${id}.scr
