...
CrashCfg::parse(AddressSpace *as, const char *str, const char **suffix)
{
	...
	for (auto it = content.begin(); it != content.end(); it++) {
		...
		if (this->content[it->first]->content[0] == 0xc3) {
			/* Special case for the return instruction */
			...
			/* So now all of the desired successors have
			   been matched to actual successors, so
			   they've been classified as branch or
			   default or call successors.  Now find any
			   actual edges which haven't been used, so
			   that we can record that we need to kill
			   them. */
